import * as hex from "https://deno.land/std@0.152.0/encoding/hex.ts"

//
// This code is copied from the deprecated std/hash.ts as is not available in new std/crypto
//
...
export const hash = (text:string) => {
  return new Md5().update(text).toString() as Hash
}

export type Uuid = string

export const uuid = () => {
  return hash(`${Date.now()}=${Math.random()}`) as Uuid
}
